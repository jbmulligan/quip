...
Vec_Func_Array mtl_vfa_tbl[N_VEC_FUNCS]={

my_include(`veclib/gen_func_array.m4')

};

#define N_MTL_ARRAYED_VEC_FUNCS (sizeof(mtl_vfa_tbl)/sizeof(Vec_Func_Array))

void check_mtl_vfa_tbl(SINGLE_QSP_ARG_DECL)
{
//	if( N_MTL_ARRAYED_VEC_FUNCS != N_VEC_FUNCS ){
//		snprintf(ERROR_STRING,LLEN,
//	"mtl_vfa_tbl has %ld entries, expected %d!?\n",
//			N_MTL_ARRAYED_VEC_FUNCS, N_VEC_FUNCS );
//		WARN(ERROR_STRING);
////		return -1;
//	}
	assert( N_MTL_ARRAYED_VEC_FUNCS == N_VEC_FUNCS );
	check_vfa_tbl(mtl_vfa_tbl, N_MTL_ARRAYED_VEC_FUNCS);
}
