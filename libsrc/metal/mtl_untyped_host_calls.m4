...
define(`MTL_FFT_INIT',`
	if( ! mtl_fft_inited ){
		mtl_fft_inited = TRUE;
fprintf(stderr,"mtl_fft_init setting exit function\n");
		do_on_exit(mtl_fft_shutdown);
	}
')

ifdef(`FOOBAR',`

static int mtl_fft_inited=FALSE;

static void mtl_fft_shutdown(SINGLE_QSP_ARG_DECL)
{
#ifdef HAVE_CLFFT
	cl_int err;
	err=clfftTeardown();
