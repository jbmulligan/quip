...
#define DECLARE_INT_FORMAT_FUNC(type_str)				\
									\
static void _format_##type_str##_value(QSP_ARG_DECL  char *buf, int buflen, Scalar_Value *svp, int pad_flag)	\
{									\
	Integer_Output_Fmt *iof_p;					\
									\
	iof_p = curr_output_int_fmt_p;					\
	(*(iof_p->iof_fmt_##type_str##_func))(QSP_ARG  buf,buflen,	\
							svp,pad_flag);	\
									\
}
...
{									\
	if( pad_flag ){							\
		snprintf(buf,buflen,padded_flt_fmt_str,svp->member);	\
	} else {							\
		snprintf(buf,buflen,"%g",svp->member);			\
	}								\
}
...
"CAUTIOUS:  '%s' is not a machine precision, can't format value!?",	\
							#type_str);	\
	error1(ERROR_STRING);						\
}
...
"%s_set_value_from_input:  Truncation error converting %s to %s (%s)",	\
		...
		warn(ERROR_STRING);					\
		snprintf(ERROR_STRING,LLEN,				\
		"val = %ld, type_min = %ld, type_max = %ld",		\
...
{									\
	...
	if( val < (-type_max) || val > type_max ){			\
		snprintf(ERROR_STRING,LLEN,				\
"%s_set_value_from_input:  Truncation error converting %s to %s (%s)",	\
		#quip_type_name,#read_type,#quip_type_name,#type);	\
		warn(ERROR_STRING);					\
	}								\
									\
	if( (val < (type_min) && val > 0) ||				\
				(val > (-type_min) && val < 0) ){	\
		snprintf(ERROR_STRING,LLEN,				\
			"Rounding error converting %s to %s (%s)",	\
			#read_type,#quip_type_name,#type);		\
		warn(ERROR_STRING);					\
	}								\
									\
	if( vp != NULL )						\
...
static void _##quip_type_name##_extract_scalar(QSP_ARG_DECL  Scalar_Value *svp, Data_Obj *dp)	\
{									\
	snprintf(ERROR_STRING,LLEN,"Nonsensical call to %s_extract_scalar!?", #quip_type_name);\
	warn(ERROR_STRING);						\
}
...
{									\
	snprintf(ERROR_STRING,LLEN,					\
"cast_indexed_%s_from_double:  Can't cast to %s with an index (%d)!?",	\
			#quip_type_name,#quip_type_name,idx);		\
	warn(ERROR_STRING);						\
}
